...
def ratkaise_lampotila(p, V, n):
    return p * V / (n * R)
```
Luodaan moduulin [*ideaalikaasu.py*](./ideaalikaasu.py) kanssa samaan hakemistoon tiedosto 
[*testi.py*](./testi.py), jossa hyödynnämme *ideaalikaasu*-moduulia **import**-avainsanan avulla:
```{code-cell} ipython3
...
```
Toinen tapa on tuoda *ideaalikaasu*-moduulista vain tietyt funktiot ja muuttujat [*testi.py*](./testi.py)-ohjelman
